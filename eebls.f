...
c
     ...
     &nb2,jn1,jn2
      real*8 :: rn,tot,s,t1,f0,p0,ph,power,rn1,pow,rn3,s3,temp_depth
...
c
      ...
      p(jf) = power
c
C     if(power.lt.bpow) go to 100

C     2015 Jun 29 -- Brian Jackson added a check to require that signal at bper corresponds to a DIP
C       and not a blip. The original code doesn't distinguish.

      temp_depth = -s3*rn/(rn3*(rn-rn3))
      if((power.lt.bpow).or.(temp_depth.lt.0)) go to 100
      bpow  =  power
      in1   =  jn1
      in2   =  jn2
      qtran =  rn3/rn
